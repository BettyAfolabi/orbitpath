...
export default function FeedbackBar({ roadmapId }: Props) {
  ...
  // Check for existing label on mount
  useEffect(() => {
  ...
  async function submit(t: Thumb, s: number) {
    if (!t || !s || status === "saved") return; // block re-rating
    setStatus("saving");
    try {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-white/10 bg-[#080B14]/90 backdrop-blur-md">
      <div className="mx-auto flex max-w-3xl items-center justify-between gap-4 px-6 py-4">
        ...
        <Link
          href="/crew"
          className="rounded-full bg-sky-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-sky-400 whitespace-nowrap"
        >
          Meet the crew →
        </Link>
